...
export class HttpClient {
  ...
  constructor(token: string, private readonly transport: AxiosInstance = axios.create({
    ...
    this.authorization = `bearer${ensureToken(token)}`;
  }
  ...
  async post(path: string, payload: object): Promise<unknown> {
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        const response = await this.transport.post(path, { ...payload, authorization: this.authorization }, {
          headers: { authorization: this.authorization }, timeout: 8000, maxRedirects: 0,
        });
        ...
        if (!confirm && transient && attempt < 2) { await this.sleep(500 * 2 ** attempt); continue; }
